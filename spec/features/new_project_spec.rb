...
RSpec.describe "Suspend a new project with default configuration" do
  ...
  it "uses custom Gemfile" do
    ...
    expect(gemfile_file).to match(
      /^ruby "#{Kimoner::RUBY_VERSION}"$/,
    )
    expect(gemfile_file).to match(
      /^gem "autoprefixer-rails"$/,
    )
    expect(gemfile_file).to match(
      /^gem "rails", "#{Kimoner::RAILS_VERSION}"$/,
    )
  end
  ...
  it "creates .ruby-version from Kimoner .ruby-version" do
  ...
  it "adds custom flash types for :error and :success" do
    controller = read_project_file("app/controllers/application_controller.rb")

    expect(controller).to match(/add_flash_types :error, :success/)
  end
